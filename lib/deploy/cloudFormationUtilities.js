...
exports.generateCloudFormationDeployConfig = function (config) {
  ...
  // Are we in fact cleaning up the wreckage of failure and deleting prior
  // application instances, as is the default behavior?
  if (config.deployment.skipPriorCloudFormationStackDeletion) {
    cfdConfig.priorInstance = cloudFormationDeploy.priorInstance.DO_NOTHING;
  }
  if (config.deployment.skipCloudFormationStackDeletionOnFailure) {
    cfdConfig.onDeployFailure = cloudFormationDeploy.onDeployFailure.DO_NOTHING;
  }
