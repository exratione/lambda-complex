  ...
  return function (stackDescription, callback) {
    ...
    async.series({
      ...
      startApplication: function (asyncCallback) {
        ...
        // This is parallel, but there should never be so much coordinator
        // concurrency that this becomes a problem.
        async.times(
          config.coordinator.coordinatorConcurrency,
          function (index, innerAsyncCallback) {
            utilities.invoke(coordinatorArn, event, innerAsyncCallback);
          },
          asyncCallback
        );
      },
