...
exports.getInvocationCounts = function (status) {
  return _.chain(status.components).filter(function (component) {
    // This is only a number for event from message components, and if the
    // check on the queue size worked.
    return typeof component.queuedMessageCount === 'number';
  }).map(function (component) {
    return {
      name: component.name,
      count: component.queuedMessageCount
    };
  }).value();
};
 ...
 * ]
 *
 * @param  {[type]} invocationCounts [description]
