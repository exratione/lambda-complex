...
/**
 * Every role is given an additional policy allowing permissions relating to S3
 * keys that are a part of the application.
 *
 * @param {Object} config The application configuration.
 * @return {Object} The statement.
 */
function getS3Statement (config) {
  var statement = {
    Effect: 'Allow',
    Action: [
      's3:GetObject'
 ...
 *
 * Each role is associated with one or more Lamdba functions and provide
...
function setRoles (template, config) {
  ...
  _.each(roleConfigurations, function (roleConfig) {
    ...
    // Lambda functions.
    statements.push(
      getQueuesStatement(config)
    );
    statements.push(
      getLambdaFunctionsStatement(config)
    );
    statements.push(
      getS3Statement(config)
    );
    statements.push(
      getCloudWatchLogsStatement(config)
    );
...
/**
 * Add the SQS queue resources to the template.
 *
 * @param {Object} template The CloudFormation template under construction.
 * @param {Object} config The application configuration.
 */
function setQueues (template, config) {
